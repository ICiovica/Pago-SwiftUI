...
class UserDetailsModelTests: XCTestCase {
    ...
    func testInvalidUpdateWithEmptyDetails() {
        let userDetails = UserDetailsModel(firstName: "John", lastName: "Doe", phone: "1234567890", email: "john@example.com")
        let user = UserModel(id: "1", name: "John Doe", email: "john@example.com", phone: "1234567890")
        
        XCTAssertTrue(userDetails.invalidUpdate(user), "Update should be invalid when details are the same with user")
    }
    
    func testInvalidUpdateWithInvalidFirstName() {
        let userDetails = UserDetailsModel(firstName: "", lastName: "Doe", phone: "1234567890", email: "john@example.com")
        let user = UserModel(id: "1", name: "John Doe", email: "john@example.com", phone: "1234567890")
        
        XCTAssertFalse(userDetails.invalidUpdate(user), "Update should be valid with mismatched details")
    }
    
    func testInvalidUpdateWithMismatchedDetails() {
        let userDetails = UserDetailsModel(firstName: "Jane", lastName: "Doe", phone: "9876543210", email: "jane@example.com")
        let user = UserModel(id: "1", name: "John Doe", email: "john@example.com", phone: "1234567890")
